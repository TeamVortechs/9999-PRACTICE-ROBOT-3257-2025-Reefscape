...
public class Elevator extends SubsystemBase{
    ...
    //Variables the encompass the elevator as a whole. The annotation means it is automatically logged in advantage kti
    @AutoLogOutput
    ...
    //PID controller so we don't need to do the logic ourselves. It just gets all of it's values from preferences
    ProfiledPIDController elevatorPID = new ProfiledPIDController(KDoublePreferences.PElevator.proportional.getValue(), 
    KDoublePreferences.PElevator.integral.getValue(), KDoublePreferences.PElevator.derivative.getValue(), 
    new TrapezoidProfile.Constraints(KDoublePreferences.PElevator.speedlimit.getValue(), KDoublePreferences.PElevator.accelerationLimit.getValue()));
