...
public class VortechsPoseEstimator extends SwerveDrivePoseEstimator {
  ...
  //this just changes the code to not update the pose if the robot detects no change in acceleration, pretty simple change but I think it could be pretty good.
  // @Override
  ...
  //   Pose2d curPose = new Pose2d(getEstimatedPosition().getX(), getEstimatedPosition().getY(), getEstimatedPosition().getRotation());
